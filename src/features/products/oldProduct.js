const oldProduct = (
    <>
            <Box onClick={() => { setClicked(true) }} className='Product'>
                <Box className='ProductIcons'>
                ...
                <Box className='ProductDetails'>
                    ...
                    <Box className='ProductExtraDetails'>
                        <Box className='ProductPriceDetails'>
                            <Box className='ProductPriceIcon'>
                            ...
                            <Box className='ProductPriceNumber'>
                                ${product.price}
                            </Box>
                        </Box>
                        <Box className='ProductFavouriteIcon'>
                            {
                                fav 
                                ?
            ...
            {
                clicked &&
                // <Box className='SingleProductContainer'>
                    <Box className='SingleProduct'>
                        <Box className='SingleProductImage'>
                            <img src={product.image} alt={product.title} />
                        </Box>
                        <Box className='SingleProductInfoAndButton'>
                            <Box className='SingleProductInfo'>
                                <Box className='SingleProductTitle'>
                                    {product.title}
                                </Box>
                                <Box className='SingleProductDesc'>
                                    {product.description}
                                </Box>
                                <Box className='SingleProductExtraInfos'>
                                    {product?.additionalInfo && Object.keys(product?.additionalInfo).map((info) => <Box className='SingleProductExtraInfo'><span>{info}</span>: {product?.additionalInfo[info]}</Box>)}
                                </Box>
                            </Box>
                            <Box className='SingleProductAddButton'>
                                <Box className='CartCount'>
                                    <Box className='CartItemUpdater'><button disabled={isLoading} onClick={() => setCount(prev => prev + 1)} >+</button></Box>
                                    <Box className='CartItemCount'>{count}</Box>
                                    <Box className='CartItemUpdater'><button disabled={isLoading} onClick={() => setCount(prev => prev - 1)}>-</button></Box>
                                </Box>
                                <button
                                    onClick={handleAdd}
                        ...
                        <Box onClick={() => { setClicked(false) }} className='ExitSingleProduct'>
                            <CloseIcon />
                        </Box>
